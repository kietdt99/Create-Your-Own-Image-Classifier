...
def save_checkpoint(file_path, model, image_datasets, epochs, optimizer, learning_rate, input_size, output_size, arch, hidden_units):
    ...
    print("Model was saved to storage")
...
def main():
    ...
    validation_set = testing_set = transforms.Compose([transforms.Resize(256),
                                                       transforms.CenterCrop(224),
                                                       transforms.ToTensor(),
                                                       transforms.Normalize([0.485, 0.456, 0.406], [0.229, 0.224, 0.225])])

    image_datasets = [datasets.ImageFolder(train_dir, transform = training_set),
                      datasets.ImageFolder(valid_dir, transform = validation_set),
                      datasets.ImageFolder(test_dir, transform = testing_set)]
